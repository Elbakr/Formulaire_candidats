...
import { Mail, Loader2, FileText, HeartHandshake, BellRing, GraduationCap } from "lucide-react";
...
import { startTrainingAction } from "./training-actions";
...
export function WorkerMailsPanel({ employeeId }: { employeeId: string }) {
  ...
  const training = useSender();
  const anyPending = sheet.pending || followup.pending || reminder.pending || training.pending;
    ...
    <Card className="p-4 space-y-2">
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <Button
          ...
          variant="gold"
          disabled={anyPending}
          title="Démarre le programme de formation gamifié (grand manuel en sections, sur ≤ 30 jours) et envoie la section du jour"
          onClick={() =>
            training.run(() => startTrainingAction(employeeId), "Formation démarrée — section envoyée.")
          }
        >
          {training.pending ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <GraduationCap className="h-3.5 w-3.5 mr-1" />}
          Démarrer / continuer la formation
        </Button>
      </div>
    </Card>
  );
