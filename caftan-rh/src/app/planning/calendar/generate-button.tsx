                ...
                {Array.from(byEmp.entries()).map(([empId, emp]) => (
                  <li key={empId} className="border border-line rounded-md p-2">
                    ...
                    <div className="text-xs text-ink-2 space-y-0.5">
                      {emp.map((s, i) => (
                        <div key={i}>
                          {new Date(s.date).toLocaleDateString("fr-BE", { timeZone: "Europe/Brussels", weekday: "short", day: "2-digit", month: "short" })} : {s.start_time} – {s.end_time} ({s.hours.toFixed(2)}h, pause {s.break_minutes}min)
                        </div>
                      ))}
                    </div>
                  </li>
