...
export default async function DimonaDashboardPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  let q = admin
    ...
    .select(`
      id, kind, employer_org_key, declared_start_date, declared_end_date,
      worker_type, status, declared_at, dimona_period_id, note, created_at,
  ...
  // Counts
  const { data: countsRaw } = await admin.from("dimona_declarations").select("status, kind");
  const counts = new Map<string, number>();
  for (const r of countsRaw ?? []) counts.set(r.status, (counts.get(r.status) ?? 0) + 1);
  const pendingIn = (countsRaw ?? []).filter((r) => r.status === "pending" && r.kind === "in").length;
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {[
          ...
          { k: "declared", l: "Déclarées", c: counts.get("declared") ?? 0 },
          { k: "failed", l: "Échec", c: counts.get("failed") ?? 0 },
      ...
      <div className="space-y-2">
        {rows.map((d) => {
          ...
          const isInDanger = d.status === "pending" && d.kind === "in";
          return (
              ...
              <div className="flex items-start gap-3 flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge className={`text-[10px] uppercase font-bold ${d.kind === "in" ? "bg-blue-100 text-blue-900" : "bg-purple-100 text-purple-900"}`}>
                      Dimona {d.kind.toUpperCase()}
                    </Badge>
                  ...
                  <div className="text-xs text-ink-3 mt-1 space-y-0.5">
                    {d.declared_start_date && (
                      <div>📅 Date <strong>{d.kind === "in" ? "entrée" : "fin"}</strong> : <span className="font-mono">{d.declared_start_date}</span>
                        {d.kind === "in" && d.declared_end_date && <span> → fin prévue : {d.declared_end_date}</span>}
                      </div>
                    )}
                    ...
                    {d.note && (
                      <div className="italic mt-1">{d.note}</div>
                    )}
                  </div>
                </div>
                <DimonaActionsRow declarationId={d.id} status={d.status} kind={d.kind} />
              </div>
            </Card>
          );
        })}
