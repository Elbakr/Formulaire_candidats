...
// Karim 2026-06-17 : capture signature du travailleur pour la convention de
// rupture amiable (pad doigt/souris), variante isolée de la signature contrat.
...
import { submitTerminationSignatureAction } from "./actions";

export function SignTerminationClient({
  terminationId,
  token,
  renderedBody,
  fullName,
  ...
  function submit() {
    ...
    startTransition(async () => {
      const r = await submitTerminationSignatureAction({ terminationId, token, signaturePng });
      if (r.error) {
        toast.error(r.error);
        return;
      }
      ...
      <Card className="border-success bg-success-light/30">
        <div className="p-6 text-center space-y-4">
          <div>
            ...
            <h2 className="text-xl font-bold text-success">Convention signée !</h2>
            <p className="text-sm text-ink-2 mt-2">
              Merci {fullName.split(" ")[0]}. La convention — avec les deux signatures — t&apos;est envoyée par mail (PDF).
              Tu la retrouveras aussi dans ton espace.
            </p>
          </div>
          <a
            href="/me/termination"
      ...
      <Card>
        <div className="p-4 space-y-3">
          ...
          <label className="flex items-start gap-2 text-xs cursor-pointer p-2 rounded bg-gold-light/30 border border-gold/30">
            ...
            />
            <span>
              J&apos;ai lu et accepté les termes de la présente convention de cessation de commun accord.
              Je confirme que ma signature ci-dessus est authentique et engage ma responsabilité.
