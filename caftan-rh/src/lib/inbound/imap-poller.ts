...
export async function pollImapInbox(opts: PollOptions = {}): Promise<PollResult> {
  ...
  // Karim 2026-06-14 : récupère les messages avec retry sur erreur IMAP
  // transitoire (« Command failed », socket, timeout). Le client est recréé à
  // chaque tentative. Le traitement (DB, déjà dédupliqué) se fait APRÈS, hors
  // connexion, pour ne jamais rejouer un import réseau si on a dû retenter.
  const messages = await retryOnTransientImap(async () => {
    ...
    const collected: Array<{ uid: number; source: Buffer | null }> = [];
    try {
      ...
      try {
        ...
        for await (const msg of client.fetch(
          ...
          if (msg.source) {
            collected.push({ uid: msg.uid as number, source: msg.source as Buffer });
          }
          if (collected.length >= maxPerRun) break;
        }
      } finally {
        lock.release();
      }
