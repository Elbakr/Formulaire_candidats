...
export async function rematchOrphanPayslips(): Promise<RematchResult> {
  ...
  for (const o of orphans) {
    ...
    // Karim 2026-07-03 : NE PAS ré-appliquer l'avance globale de l'employé à une
    // ancienne fiche re-matchée (ça vidait à tort le net — cas ElBertitan juin).
    // L'avance ne se saisit que manuellement (input). Le QR est (re)généré par
    // ensureUnpaidQrs() à partir du montant restant courant.
    res.rematched++;
